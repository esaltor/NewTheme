define(['dojo/_base/declare', 'jimu/PoolControllerMixin',
    'jimu/BaseWidget', 'dojo/on', 'dojo/query', 'dojo/dom-class'
  ],
  function(declare, PoolControllerMixin, BaseWidget, on, query, domClass) {
    return declare([BaseWidget, PoolControllerMixin], {
      ...
      _createIconNode: function(iconConfig, targetNode) {
        ...
        targetNode.appendChild(iconNode); // check if the widget is set to open at start
      ...
      _showWidgetContent: function(iconConfig) {
        ...
        var self = this;
        this.panelManager.showPanel(iconConfig).then(function(widget) { // the panel displays successfully
          self.own(on.once(widget, 'close', function() {
            domClass.remove(self.activeIconNode, 'jimu-state-active');
            self.activeIconNode = null;
          }));
        }, function(err) { // the panel failed to display
        });
